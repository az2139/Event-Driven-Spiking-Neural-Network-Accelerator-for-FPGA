...
entity snn_inference_profile_hls_ctrl_s_axi is
generic (
...
end entity snn_inference_profile_hls_ctrl_s_axi;
...
architecture behave of snn_inference_profile_hls_ctrl_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    constant ADDR_ENCODER_CONFIG_CTRL    : INTEGER := 16#54#;
    constant ADDR_STATUS_REG_DATA_0      : INTEGER := 16#58#;
    constant ADDR_STATUS_REG_CTRL        : INTEGER := 16#5c#;
    constant ADDR_SPIKE_COUNT_REG_DATA_0 : INTEGER := 16#68#;
    constant ADDR_SPIKE_COUNT_REG_CTRL   : INTEGER := 16#6c#;
    constant ADDR_WEIGHT_SUM_REG_DATA_0  : INTEGER := 16#78#;
    constant ADDR_WEIGHT_SUM_REG_CTRL    : INTEGER := 16#7c#;
    constant ADDR_VERSION_REG_DATA_0     : INTEGER := 16#88#;
    constant ADDR_VERSION_REG_CTRL       : INTEGER := 16#8c#;
    constant ADDR_REWARD_SIGNAL_DATA_0   : INTEGER := 16#98#;
    constant ADDR_REWARD_SIGNAL_CTRL     : INTEGER := 16#9c#;
    ...
    signal int_encoder_config  : UNSIGNED(79 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_ENCODER_CONFIG_DATA_2 =>
                        rdata_data <= RESIZE(int_encoder_config(79 downto 64), 32);
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_encoder_config(79 downto 64) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_ENCODER_CONFIG_DATA_2) then
                    int_encoder_config(79 downto 64) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_encoder_config(79 downto 64));
                end if;
            end if;
        end if;
    end process;
